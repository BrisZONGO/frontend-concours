...
const renderInlineFile = (block) => {
  ...
  const extension = (block.extension || "").toLowerCase();
  ...
  if (block.mimeType?.startsWith("image/")) {
    return (
      <img
        src={fileUrl}
        alt={block.fichierNom || block.titre || "fichier"}
        style={{ maxWidth: "100%", borderRadius: "6px", marginTop: "10px" }}
      />
    );
  }

  if (block.mimeType === "application/pdf" || extension === "pdf") {
    return (
      <iframe
        src={fileUrl}
        title={block.fichierNom || block.titre || "pdf"}
        style={{
          width: "100%",
          height: "500px",
  ...
  if (["doc", "docx", "ppt", "pptx", "xls", "xlsx"].includes(extension)) {
    return (
      <iframe
        ...
        title={block.fichierNom || block.titre || "office-file"}
        style={{
          width: "100%",
          height: "600px",
          border: "1px solid #ddd",
